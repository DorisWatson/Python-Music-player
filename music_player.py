...
#Functions
def openfile():
    filename = filedialog.askopenfilename(initialdir="C:/",title="Open File", filetypes=(("mp3","*.mp3"),("All Files","*.*")))
...
# Main Screen
master = Tk()
master.title("I hope this works")

#labels
Label(master,text = "Custom Music Player", font=("Calibri",15),fg="red").grid(sticky="N",row=0,padx=120)
Label(master,text = "Select select a track", font=("Calibri",12),fg="blue").grid(sticky="N",row=1)
...
#Buttons
Button(master, text="Select Song", font=("Calibri", 12),command=openfile).grid(row=2, sticky="N")
